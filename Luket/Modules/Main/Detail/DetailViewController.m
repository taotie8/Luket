...
@property (nonatomic, strong) UIImageView *commentAvatarView;
@property (nonatomic, strong) UILabel *commentPreviewLabel;
...
- (void)viewDidLayoutSubviews {
    ...
    self.imageFrameView.frame = CGRectMake(20.0 * scale, safeTop + 85, width - 40, 470.0 * scale);
    ...
    self.contentLabel.frame = CGRectMake(20.0 * scale, 630.0 * scale, 300.0 * scale, 22.0 * scale);
    self.commentsTitleLabel.frame = CGRectMake(20.0 * scale, 670.0 * scale, 170.0 * scale, 24.0 * scale);
    self.commentAvatarView.frame = CGRectMake(20.0 * scale, 704.0 * scale, 24.0 * scale, 24.0 * scale);
    self.commentAvatarView.layer.cornerRadius = 12.0 * scale;
    self.commentPreviewLabel.frame = CGRectMake(57.0 * scale, 706.0 * scale, 220.0 * scale, 20.0 * scale);

    CGFloat inputHeight = 56.0;
    self.commentInputView.frame = CGRectMake(20.0, CGRectGetHeight(self.view.bounds) - inputHeight - 29.0, width - 40.0, inputHeight);
...
- (void)configureImageArea {
    ...
    [self.view addSubview:self.imageFrameView];
...
- (void)configureTextArea {
    ...
    [self.view addSubview:self.commentsTitleLabel];

    self.commentAvatarView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"HomeHeroImage"]];
    self.commentAvatarView.contentMode = UIViewContentModeScaleAspectFill;
    self.commentAvatarView.clipsToBounds = YES;
    [self.view addSubview:self.commentAvatarView];

    self.commentPreviewLabel = [[UILabel alloc] initWithFrame:CGRectZero];
    self.commentPreviewLabel.text = @"Every time I swim, I feel ...";
    self.commentPreviewLabel.textColor = [UIColor colorWithWhite:1.0 alpha:0.8];
    self.commentPreviewLabel.font = [UIFont systemFontOfSize:13.0];
    [self.view addSubview:self.commentPreviewLabel];
}
